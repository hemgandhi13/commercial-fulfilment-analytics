...
**▶ NEXT SESSION — verify Batch 6 in Desktop, then Batch 7** (Page 05 leakage table per plan §6: parameter-swap first column [already done Batch 3], +`[Retailer Rebate Accrual]` column, discount-rate background colour scale [user-authored data bars/scale from Batch 3 may partially exist — check before adding], area chart +Rebate Accrual gold line series, card `5f544d3e` → Rebate Accrual gold; then p06/07 per plan — needs the ~30% SLA threshold confirmed) with page-logic card conditionals throughout. (Page 04 promotional ROI: margin-erosion chart second series `[Net Commercial Margin %]` + Alert-Red conditional, profit-capture conditional colours, card rebind `78d5c340` → `[Discount Amount]`, STATE slicer → SHIPPING_MODE, median-discount reference line on the scatter, optional discount trend combo) per `Docs/V2_Report_Pages_Upgrade_Plan.md` §5. Then Batches 7–9 in order, one commit per batch, Desktop verify between. Remaining business input: ~30% SLA-breach threshold (Batch 7) still unconfirmed. Note: p02 STATE slicer `0af6b5f7` → SHIPPING_MODE (WS-3) was never done in Batch 4 — fold into Batch 9 sweep.
