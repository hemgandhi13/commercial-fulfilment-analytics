...
- [x] **Batch 2.2 — Correct field-parameter binding shape captured + reference line template** (commit `3dbb323`)
  ...
  - **Manual verify round 3 (gates Batch 3):** reload .pbip → Page 02 → the two ORIGINAL charts must now swap with the Parameter slicer exactly like the test chart; CTS line must render red dashed at 12%. After confirming, the test chart `de197928e9dd6a8eab08` can be deleted (keep it until then). DO NOT start Batch 3 before user confirmation.
